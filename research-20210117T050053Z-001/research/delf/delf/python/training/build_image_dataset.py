...
flags.DEFINE_string('test_directory', None,
                    '(Optional) Testing data directory. Required only if test_csv_path is not None.')
...
def _write_tfrecord(output_prefix, image_paths, file_ids, labels):
  ...
  Args:
    ...
    labels: list of integers, the landmark ids of images. It is an empty list when output_prefix='test'.
  Raises:
  ...
  for shard in range(FLAGS.num_shards):
    output_file = os.path.join(FLAGS.output_directory, 
                  '%s-%.5d-of-%.5d.tfrecords' % (output_prefix, shard, FLAGS.num_shards))
...
def _get_all_image_files_and_labels(name, csv_path, image_dir):
  ...
  if name == _TRAIN_SPLIT:
    with tf.io.gfile.GFile(csv_path, 'rb') as csv_file:
      ...
      # Create  image_paths, file_ids ans labels
      image_paths = list()
      file_ids = list()
      labels = list()
      num_errorPath = 0
      for index, row in df.iterrows():
        ...
        image_path_dir1 = os.path.join(image_dir, image_id[0])
        image_path_dir2 = os.path.join(image_path_dir1, image_id[1])
        image_path = os.path.join(image_path_dir2, image_name)

        # Check image path exist if not raise error
        if not os.path.exists(image_path):
          num_errorPath += 1
          print("Invalid image path")

        image_label = row['landmark_id']

        image_paths.append(image_path)
        file_ids.append(image_id)
        labels.append(image_label)

      print(f'Number error image path: {num_errorPath}')
...
def _build_train_tfrecord_dataset(csv_path, image_dir, generate_train_validation_splits, 
  ...
  # Get image_paths, file_ids, labels
  ...
  if generate_train_validation_splits:
    ...
    validation_split, train_split = _build_train_and_validation_splits(image_paths, file_ids, labels, 
                                                                      validation_split_size, seed)
