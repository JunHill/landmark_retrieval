...
flags.DEFINE_string(
    'dataset_file_path', '/tmp/retrieval.csv',
...
def _get_image_name_of_dataset(dataset_file_path):
  image_list = list()
...
def main(argv):
  ...
  image_list = _get_image_name_of_dataset(FLAGS.dataset_file_path)
  ...
  for i in range(num_images):
    ...
    input_image_filename = os.path.join(FLAGS.images_dir, image_name + _IMAGE_EXTENSION)
    ...
    if config.use_global_features:
      ...
      if i < 20:
        print(f'Global descriptors of image {image_list[i]}: {global_descriptor}')
    ...
    if config.use_local_features:
      ...
      if i < 20:
        print(f'Locations of local feature of image {image_list[i]}: {len(locations)}')
        print(f'Descriptors of local feature of image {image_list[i]}: {len(descriptors)}')
